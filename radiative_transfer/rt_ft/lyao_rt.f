...
C ATOMIC MASS
      ...
      DIMENSION TSOL(IKNT,JKNT),SOURCE(MDIM)
      DIMENSION LUI(MDIM),ASRC(MDIM),SRC(IKNT,JKNT)
      DIMENSION OSUN(3)
C     LARGE OR PER-THREAD ARRAYS LIVE ON THE HEAP (-fopenmp PUTS PLAIN
C     LOCAL ARRAYS ON THE STACK)
      REAL*8, ALLOCATABLE :: HOMO(:,:),ALUD(:,:)
      REAL*8, ALLOCATABLE :: ODLC(:,:,:,:,:),ODO2(:,:,:,:),
     &                       STORE(:,:,:,:)
      LOGICAL, ALLOCATABLE :: TRIP(:,:)
...
C      COMMON/PARA_MSIS/ GLAT,GLONG,IDAY,IYEAR,UT,STL,IAPH,AP,F107,F107A
      ...
      COMMON /EXOS/  GM,CONL,PLANETR,RBASE,RC,RUPR,RP,TEXO,DEXO,
     &               PHZ_SHFT,TOGGLE,VELT,RADPF,CENTER,ABSCSX,FTSAT,
     &               FDSAT,IGEO
...
C NOTE: Solar angle ==> Angle from planet-sun axis
...
C . . . MANY ENTRIES ARE ZERO, SO BEGIN WITH A ZERO MATRIX
      ALLOCATE(HOMO(MDIM,MDIM),ALUD(MDIM,MDIM))
      DO 1000 MROW = 1,MDIM
      DO 1000 MCOL = 1,MDIM
...
C EACH BIN FILLS ONLY ITS OWN ROW OF HOMO, SO THE BINS ARE SPREAD OVER
C OPENMP THREADS, EACH WITH ITS OWN ZONE STORAGE ARRAYS
!$OMP PARALLEL DEFAULT(NONE)
!$OMP& SHARED(HOMO,RADN,CHIN,TEMP,BRAD,BCHI,HDNS,O2DNS,SPDPT,WSPD,
!$OMP&        THERMO,ITHERM,WPHI,NONIT,/EXOS/,/NMBR/,/GAUS08/,/GAUS16/,
!$OMP&        /GAUS24/,/GAUS32/,/GAUS64/,/GAUS96/)
!$OMP& PRIVATE(TRIP,ODLC,ODO2,STORE,MROW,II,JJ,R1,X1,FCTREF,KMU,THETA,
!$OMP&         KPHI,PHI,IEXIT,JEXIT,III,JJJ,RHO,ABS11,ABS21,TAU1,TAU2,
!$OMP&         TLC11,TLC21,LP,XPNT,HOLDIT,SUM,KK,TAU,WTAU,TABS,FCTR,
!$OMP&         TSCAT,TAULP,ABS12,ABS22,TLC12,TLC22,MCOL,SUM1,WUMU,SUM2)
      ALLOCATE(TRIP(IKNT,JKNT),ODLC(2,0 : LPROF,IKNT,JKNT,2),
     &         ODO2(2,IKNT,JKNT,2),STORE(LMU,LPHI,IKNT,JKNT))
C ZONE STORAGE ARRAYS START CLEARED; EACH RAY'S CELLS ARE RESET AFTER USE
      TRIP = .FALSE.
      ODLC = 0.0D0
      ODO2 = 0.0D0
!$OMP DO SCHEDULE(DYNAMIC)
      DO 1001 MROW = 1,MDIM
        II   = MOD(MROW-1,IKNT) + 1
        JJ   = (MROW-1)/IKNT + 1
        R1   = RADN(II)
        X1   = CHIN(JJ)
        FCTREF = TEXO/TEMP(II)
...
C . . . . . . . . RESET THIS CELL FOR THE NEXT CALL TO ZONE
                TRIP(III,JJJ) = .FALSE.
                ODO2(:,III,JJJ,:)   = 0.0D0
                ODLC(:,:,III,JJJ,:) = 0.0D0
              ELSE 
...
C LAPACK LU FACTORIZATION (REPLACES NUMERICAL RECIPES LUDCMP).  SAME
C PACKED L\U STORAGE AND SEQUENTIAL ROW-SWAP PIVOTS, SO LUBKSB AND
C MPROVE BELOW WORK UNCHANGED.
      CALL DGETRF(MDIM,MDIM,ALUD,MDIM,LUI,INFO)
      IF (INFO.NE.0) STOP 'Singular matrix (DGETRF).'
