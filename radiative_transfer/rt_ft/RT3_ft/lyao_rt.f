...
C simple driver for batch execution:  lyao_rt
      ...
      CALL global_parameters !exosvar
      CALL LYAO_RT
      END
...
C PRELIMINARY
      ...
      PARAMETER (IKNT  =  32)                ! FIXED 89
      PARAMETER (JKNT  =  32)                ! MUST BE EVEN
      PARAMETER (MDIM  = 1024)                ! IKNT * JKNT 2848
      PARAMETER (LMU   =  24)                ! OPTIONS: 4 8 16 24 32 64 96
      PARAMETER (LPHI  =  16)                ! MUST BE EVEN
      PARAMETER (LPROF =   8)                ! OPTIONS: 4 8 16
      PARAMETER (IEXO  =  14)                ! EXOBASE INDEX IN RADIAL BINNING
...
C      DATA GM       / 3.9898D20/         ! G-CONSTANT * PLANET MASS
C      DATA PLANETR  / 6371.0D5 /         ! MEAN PLANET RADIUS
C      DATA OFFSET   / 1.0D-6   /         ! GENERIC OFFSET VALUE

C        PRINT*,'Strt=',MOD_MSIS, D_EXO, FLUX, D_MAX
C        PRINT*,'Etrt=',IGEO, SATT, SATD, ALT_MAX
C        PRINT*,LINE_LABEL,WAVELN,FNUMBER,BRATIO,ABS_N2,ABS_O2
C        PRINT*,'doy=',IDAY,IYEAR

      OPEN (10,FILE='LYAO_source.DAT',STATUS='UNKNOWN')
...
C      PRINT*,'year===',IDAY,IYEAR
C      WRITE(10,10)CHOOZ,ITORNIT,
C     &            GLAT,GLONG,IDAY,IYEAR,UT,STL,AP(1),F107,F107A,HISTORY
 10   FORMAT(1X,'SOURCE EVALUATION CONDITIONS: ',A,2X,A/
     &       1X,F6.2,2X,F7.2,2X,I3,2X,I4,2X,5(F6.2,2X),2X,A)

C THERMOSPHERE REFERENCE TABLE
      WRITE(10,11)BASE,TOP,ITHERM,MOD_MSIS,SCALN,ADJT,D_EXO,FLUX,D_MAX
 11   FORMAT(1X,'MSIS-90 THERMOSPHERE:'/1X,0P2F9.3,2I4,0P2F9.3,1P3E12.3)
 12   FORMAT(1X,5(1PE10.4,1X))
      ...
      DO 102 IBIN = 1,ITHERM+1
        THERMO(1,IBIN) = THERMO_MSIS(1,IBIN)                     ! z    (km)
        THERMO(2,IBIN) = THERMO_MSIS(2,IBIN)                     ! r    (cm)
        THERMO(3,IBIN) = THERMO_MSIS(3,IBIN)                     ! T    (K)
        THERMO(4,IBIN) = THERMO_MSIS(4,IBIN)                     ! [H]  (cm^-3)
        THERMO(5,IBIN) = THERMO_MSIS(5,IBIN)                     ! [O2] (cm^-3)
     &                   + (ABS_N2/ABS_O2)*THERMO_MSIS(6,IBIN)   ! [N2] (cm^-3)
        WRITE(10,12)(THERMO(JJ,IBIN),JJ=1,5)
 102  CONTINUE

      BRANCH = DBLE(BRATIO)

C WRITE OUT EXOSPHERE & OPTICAL QUANTITIES
      WRITE(10,13)IGEO,TEXO,DEXO,TSAT,DSAT,RBASE,RC,RUPR,RP,
     &            RADPF,VELT,CENTER,ABSCSX,BRANCH
 13   FORMAT(1X,'EXOSPHERE & OPTICAL QUANTITIES:'/
     &       I3,4(1X,1PE10.4)/10(1X,1PE10.4))
...
C . . . WRITE OUT
      WRITE(10,14)COLHT,COLOT,COLTOT,COLHE
 14   FORMAT(1X,'COLUMN DENSITIES:'/1X,4(1PE10.4,2X))
...
C . . . DEFINE "NOMINAL" CENTROID RADII
 ...
 302  CONTINUE
      ...
      DO JJ=1,IKNT
C        PRINT*,'BRAD=',JJ,(BRAD(JJ)-PLANETR)*1D-5
      END DO
...
C WRITE OUT RADIAL AND SOLAR ANGLE GRIDS
      WRITE(10,15)BRAD
 15   FORMAT(1X,'RADIAL GRID INFO:'/(10(1X,1PE10.4)))
      WRITE(10,16)BCHI
 16   FORMAT(1X,'SOLAR ANGLE GRID INFO:'/(10(1X,1PE10.4)))

C WRITE OUT "NOMINAL" CENTROID RADII AND SOLAR ANGLES
      WRITE(10,17)RADN
 17   FORMAT(1X,'NOMINAL CENTROID RADII:'/(10(1X,1PE10.4)))
      WRITE(10,18)CHIN
 18   FORMAT(1X,'NOMINAL CENTROID SOLAR ANGLES:'/(10(1X,1PE10.4)))
...
C MSIS THERMOSPHERE:  PLANE PARALLEL, QUASI-ISOTHERMAL TREATMENT
 ...
 411      CONTINUE
          ...
          ZSOFAR = BRAD(II+1)
C          PRINT*,'ZSOFAR=',II,I_STEP(II),BRAD(II+1)*1D-5-6371!,ZSOFAR*1D-5
          II = II + 1
...
C WRITE OUT DENSITY & TEMPERATURE ARRAYS
      WRITE(10,21)HDNS
 21   FORMAT(1X,'HYDROGEN DENSITIES:'/(10(1X,1PE10.4)))
      WRITE(10,22)O2DNS
 22   FORMAT(1X,'MOLECULAR OXYGEN DENSITIES:'/(10(1X,1PE10.4)))
      WRITE(10,23)TEMP
 23   FORMAT(1X,'TEMPERATURES:'/(10(1X,1PE10.4)))
...
C WRITE OUT SOLAR TRANSMISSION FUNCTIONS
      WRITE(10,31)
 31   FORMAT(1X,'SOLAR TRANSMISSION FUNCTIONS:')
 32   FORMAT(10(1X,1PE10.4))
      DO 502 II = 1,IKNT
        WRITE(10,32)(TSOL(II,JJ),JJ=1,JKNT)
 502  CONTINUE
...
C WRITE OUT TOTAL SOURCE FUNCTIONS
      WRITE(10,33)
 33   FORMAT(1X,'TOTAL SOURCE FUNCTIONS:')
 34   FORMAT(10(1X,1PE10.4))
      DO 503 II = 1,IKNT
        WRITE(10,34)(SRC(II,JJ),JJ=1,JKNT)
 503  CONTINUE
