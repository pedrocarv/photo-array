...
C STORAGE ARRAYS TRIP, ODLC & ODO2 MUST BE ZERO/FALSE ON ENTRY.  ONLY CELLS
C WITH TRIP = .TRUE. ARE WRITTEN, SO THE CALLER RESETS JUST THOSE CELLS
C AFTER USING THEM INSTEAD OF CLEARING THE WHOLE ARRAYS FOR EVERY RAY.
...
C DETERMINATION OF LOCAL DENSITIES & TEMPERATURES USING MSIS90 TABLE
      ...
      SUBROUTINE DLOCAL(ITHERM,THERMO,RL,ANG,HLOC,O2LOC,TLOC)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION THERMO(5,ITHERM+1)
      COMMON /EXOS/  GM,CONL,PLANETR,RBASE,RC,RUPR,RP,TEXO,DEXO,
     &               PHZ_SHFT,TOGGLE,VELT,RADPF,CENTER,ABSCSX,FTSAT,
...
C  EXOSPHERE
C NOTE: For now, assuming average density and temp is at 3pi/4 solar angle
      IF (RL.GT.(RC*(1.0d0 + OFFSET))) THEN
        CALL CORONA(RL,ANG,HLOC)
        O2LOC = 0.0D0
        t_shift = PHZ_SHFT + (PI / 2)
        TLOC = TEXO + TOGGLE * TEXO * 0.3 *(1 + SIN(ANG + t_shift))/2
...
C                                AT LOCAL TEMPERATURE
      ...
      SUBROUTINE STEP_TAU(ITHERM,THERMO,R1,X1,THETA,D2,D1,
 ...
 101  END DO
      ...
      DO 201 ITAU = 1,8
        ...
        CALL DLOCAL(ITHERM,THERMO,RTL,X1,HLOC,O2LOC,TLOC)
...
C               THE XSMS AND CHIN ARRAYS.
     ...
     &                      XSMS,D0SMS,D2SMS)
      IMPLICIT REAL*8 (A-H,O-Z)
C     NMAX must be at least as large as IKNT+2
...
C =============================================================================
C =============================================================================
C INDEX FINDING SUBROUTING: GET THE INDEX OF THE CLOSEST GRIDPOINT TO AN ANGLE
C ANG - Angle, SAG - Solar Angle Grid (of exobase), SAX - SOlar Angle grid size
C IND - Index of closest angle
C By JC
      SUBROUTINE CLOSE_CHI(ANG, SAG, SAX, IND)
        ! do something here
      END
