...
typedef int(__stdcall *AtikMethod_I_HS)(		 ArtemisHandle, const char**);
...
typedef int(__stdcall *AtikMethod_I_HIUSfF)(	 ArtemisHandle, int, unsigned short ,float *, float);
...
typedef bool(__stdcall * AtikMethod_ArtemisSetSetFastCallbackEx)(ArtemisHandle, void(*callback)(ArtemisHandle handle, int x, int y, int w, int h, int binx, int biny, void * imageBuffer, unsigned char * info));
...
extern "C"
{
	...
	artfn int ArtemisCameraSerialEx(ArtemisHandle handle, const char** serial)
	{
		static AtikMethod_I_HS method = NULL;

		if (method == NULL)
			method = (AtikMethod_I_HS)GetProcAddress(hArtemisDLL, "ArtemisCameraSerialEx");

		if (method == NULL)
			return FALSE;

		return method(handle, serial);
	}
	...
	int ArtemisAutoExposureLength(ArtemisHandle handle, int percentile, unsigned short adu, float* exposureLength, float startingExposureLength)
	{
		static AtikMethod_I_HIUSfF method = NULL;

		if (method == NULL)
			method = (AtikMethod_I_HIUSfF)GetProcAddress(hArtemisDLL, "ArtemisAutoExposureLength");

		if (method == NULL)
			return FALSE;

		return method(handle, percentile, adu, exposureLength, startingExposureLength);
	}

	int ArtemisSetRegionsOfInterest(ArtemisHandle handle, AtikROI const* ROIs, int numROIs)
	{
		using ArtemisSetRegionsOfInterest_fn_ptr = int(__stdcall*)(ArtemisHandle handle, AtikROI const* ROIs, int numROIs);
		static ArtemisSetRegionsOfInterest_fn_ptr method = NULL;

		if (method == NULL)
			method = (ArtemisSetRegionsOfInterest_fn_ptr)GetProcAddress(hArtemisDLL, "ArtemisSetRegionsOfInterest");

		if (method == NULL)
			return FALSE;

		return method(handle, ROIs, numROIs);
	}

	int ArtemisGetRegionsOfInterest(ArtemisHandle handle, AtikROI const** ROIs, int* numROIs)
	{
		using ArtemisGetRegionsOfInterest_fn_ptr = int(__stdcall*)(ArtemisHandle handle, AtikROI const** ROIs, int* numROIs);
		static ArtemisGetRegionsOfInterest_fn_ptr method = NULL;

		if (method == NULL)
			method = (ArtemisGetRegionsOfInterest_fn_ptr)GetProcAddress(hArtemisDLL, "ArtemisGetRegionsOfInterest");
	...
	artfn BOOL  ArtemisImageFailed(ArtemisHandle handle)
	{
		...
		if (method == NULL)
			method = (AtikMethod_B_H)GetProcAddress(hArtemisDLL, "ArtemisImageFailed");
	...
	{
		static AtikMethod_ArtemisSetSetFastCallbackEx method = NULL;

		if (method == NULL)
			method = (AtikMethod_ArtemisSetSetFastCallbackEx)GetProcAddress(hArtemisDLL, "ArtemisSetFastCallbackEx");
